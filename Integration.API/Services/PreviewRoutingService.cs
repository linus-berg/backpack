...
public class PreviewRoutingService
{
  ...
  public async Task<ArtifactPreviewResponse> GetDataDynamicallyAsync(string target_queue_name, ArtifactPreviewRequest request_data)
  {
    // 1. Construct the destination URI. 
    // Using the "queue:" scheme is the modern, transport-independent way in MassTransit 8+.
    Uri destination_address = new Uri($"queue:{target_queue_name}");
    ...
    Response<ArtifactPreviewResponse> response = await client.GetResponse<ArtifactPreviewResponse>(request_data);
