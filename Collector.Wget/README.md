...
Collector.Wget is a specialized collector that uses the `wget` CLI tool for retrieving artifacts over HTTP and HTTPS protocols.

## Key Components

- **WgetConsumer**: Implements `IConsumer<ArtifactCollectRequest>`, handling collection requests for generic resources using `wget`.
